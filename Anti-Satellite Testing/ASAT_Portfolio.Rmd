...
tests_per_year$Year <- as.numeric(as.character(tests_per_year$Year))
...
# 4. THE FIX: Render to file manually
# By setting 'file', we bypass the internal 'if' check that causes the crash
if(!file.exists("total_ASAT.gif")){
  animate(anim_obj, 
          duration = 10, 
          fps = 10, 
          width = 800, 
          height = 600, 
          renderer = gifski_renderer("total_ASAT.gif"))
}

# 5. Display the file
knitr::include_graphics("total_ASAT.gif")
```
...
launch_data <- launch_data %>%
  mutate(
    Launch_lat = as.numeric(Launch_lat),
    Launch_long = as.numeric(Launch_long)
  ) %>%
  filter(!is.na(Launch_long))
...
# 3. Size by Count
site_counts <- launch_data %>% 
  group_by(Launch_Site) %>% 
  summarize(Count = n(), .groups = "drop")

launch_data <- launch_data %>% left_join(site_counts, by = "Launch_Site")

# 4. Plot
ggplot() +
  geom_polygon(data = my_world_map, aes(x = long, y = lat, group = group), 
               fill = "grey90", color = "black") +
  geom_point(data = launch_data, 
             aes(x = Launch_long, y = Launch_lat, color = Country, size = Count)) +
  scale_size_continuous(range = c(2, 8), name = "Number of Tests") + 
  scale_color_manual(values = c(Russia = "darkred", China = "gold", USA = "darkblue", India = "darkgreen")) +
  coord_equal(ratio = 1) +
  labs(title = "ASAT Test Launch Locations") +
  theme_minimal() +
  theme(
    legend.position = "top",
    plot.background = element_rect(fill = "grey34"),
